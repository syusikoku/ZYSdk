...
public class PermissionCheckUtils {
     ...
     * 数组
     */
    public static boolean checkPermission(Activity activity, String permission, String permissionDesc, int
            requestCode) {
        int currentAPIVersion = Build.VERSION.SDK_INT;
        if (currentAPIVersion >= android.os.Build.VERSION_CODES.M) {
            if (ContextCompat.checkSelfPermission(activity, permission) != PackageManager.PERMISSION_GRANTED) {
                LoggerUtils.loge("ContextCompat.checkSelfPermission(activity, permission):" + ContextCompat
                        .checkSelfPermission(activity, permission));
                LoggerUtils.loge("PackageManager.PERMISSION_GRANTED:" + PackageManager.PERMISSION_GRANTED);
                LoggerUtils.loge("permission:" + permission);
